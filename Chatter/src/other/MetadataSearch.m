...
{
	MetadataSearch *search = [[MetadataSearch alloc] init];
	
	search.contentType = contentType;
	search.typeCode = typeCode;
	search.kind = kind;
	search.name = name;
	search.handler = [handler copy];
...
- (void)start:(MetadataSearch *)search
{
	@autoreleasepool {
		...
		if ([predicates count] == 1)
			mPredicate = [predicates objectAtIndex:0];
		else
			mPredicate = [NSCompoundPredicate orPredicateWithSubpredicates:predicates];
